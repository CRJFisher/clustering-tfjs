...
async function findBestK() {
  ...
  // Also works with SOM
  const som_result = await find_optimal_clusters(data, {
    min_clusters: 4,
    max_clusters: 16,
    algorithm: 'som',
    algorithm_params: {
  ...
  console.log(
    `Davies-Bouldin index: ${result.optimal.davies_bouldin.toFixed(3)}`,
  );
...
async function spectralExample() {
  ...
  const generateHalfMoon = (
    ...
    for (let i = 0; i < n; i++) {
      ...
      const x =
        radius * Math.cos(angle) + (Math.random() - 0.5) * noise + offset[0];
      const y =
        radius * Math.sin(angle) + (Math.random() - 0.5) * noise + offset[1];
      points.push([x, y]);
    }
    return points;
  };
...
async function hierarchicalExample() {
  ...
  const normalize = (data: number[][]) => {
    const means = data[0].map(
      (_, i) => data.reduce((sum, row) => sum + row[i], 0) / data.length,
    );
    const stds = data[0].map((_, i) =>
      Math.sqrt(
        data.reduce((sum, row) => sum + Math.pow(row[i] - means[i], 2), 0) /
          data.length,
      ),
    );

    return data.map((row) => row.map((val, i) => (val - means[i]) / stds[i]));
  };
...
async function evaluateClustering() {
  ...
  console.log(
    `Silhouette Score: ${silhouette.toFixed(3)} (higher is better, range: [-1, 1])`,
  );
  console.log(`Davies-Bouldin Index: ${davies.toFixed(3)} (lower is better)`);
  console.log(
    `Calinski-Harabasz Index: ${calinski.toFixed(3)} (higher is better)`,
  );
...
async function onlineLearning() {
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    const quantError = som.quantizationError();
    console.log(
      `Quantization error after batch ${i + 1}: ${quantError.toFixed(4)}`,
    );
  }
