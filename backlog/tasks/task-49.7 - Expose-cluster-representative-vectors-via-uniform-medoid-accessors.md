...
<!-- SECTION:NOTES:BEGIN -->
ClusterRepresentations interface; select_medoids (O(n*d), reuses tensor_ops metrics, -1 for empty clusters); Agglomerative/Spectral compute_medoids + medoid_indices_. generate_agglomerative_medoids.py fixtures with per-sample distances for tie resolution.
<!-- SECTION:NOTES:END -->
