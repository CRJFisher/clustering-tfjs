...
describe("pairwise_distance_matrix", () => {
  const points = tf.tensor2d([[0, 0], [3, 4], [6, 8]], [3, 2]);
  afterAll(() => points.dispose());

  it("euclidean metric produces correct pairwise distances", () => {
    const result = pairwise_distance_matrix(points, "euclidean");
    const dist_mat = result.arraySync() as number[][];
    result.dispose();
  ...
  it("euclidean matrix is symmetric with zero diagonal", () => {
    ...
    const result = pairwise_distance_matrix(pts as tf.Tensor2D, "euclidean");
    const dist = result.arraySync() as number[][];
    result.dispose();
    pts.dispose();
  ...
  it("computes 100×50 matrix without crashing (performance smoke)", () => {
    const big = tf.randomUniform([100, 50]);
    const start = Date.now();
    const result = pairwise_distance_matrix(big as tf.Tensor2D, "euclidean");
  ...
  it("manhattan metric produces ℓ1 distances", () => {
    const result = pairwise_distance_matrix(points, "manhattan");
    const dist_mat = result.arraySync() as number[][];
    result.dispose();
  ...
  it("manhattan matrix is symmetric with zero diagonal", () => {
    const pts = tf.randomUniform([5, 4]);
    const result = pairwise_distance_matrix(pts as tf.Tensor2D, "manhattan");
    const dist = result.arraySync() as number[][];
    result.dispose();
    pts.dispose();
  ...
  it("cosine metric: diagonal is 0, all values in [0, 2]", () => {
    const result = pairwise_distance_matrix(points, "cosine");
    const dist_mat = result.arraySync() as number[][];
    result.dispose();
    ...
    for (let i = 0; i < points.shape[0]; i++) {
      ...
      for (let j = 0; j < points.shape[0]; j++) {
        ...
        expect(dist_mat[i][j]).toBeLessThanOrEqual(2);
      }
    }
  });

  it("cosine metric encodes angle: orthogonal→1, opposite→2, scaled→0", () => {
    const vecs = tf.tensor2d([[1, 0], [0, 1], [-1, 0], [5, 0]]);
    const result = pairwise_distance_matrix(vecs, "cosine");
    const d = result.arraySync() as number[][];
    result.dispose();
    vecs.dispose();
  ...
  it("cosine matrix is symmetric with zero diagonal", () => {
    const pts = tf.tensor2d([[1, 2, 3], [4, 0, 1], [0, 1, 0], [2, 2, 2]]);
    const result = pairwise_distance_matrix(pts, "cosine");
    const dist = result.arraySync() as number[][];
    result.dispose();
    pts.dispose();
  ...
  it("euclidean clamps coincident points to 0 (no NaN from sqrt of negative float32)", () => {
    // Duplicate rows drive ‖x‖²+‖y‖²−2xᵀy slightly negative in float32; the
    // tf.maximum(.,0) guard must keep the distance real and zero.
    const dups = tf.tensor2d([[1, 2], [1, 2], [4, 6]]);
    const result = pairwise_distance_matrix(dups, "euclidean");
    const dist = result.arraySync() as number[][];
    result.dispose();
    dups.dispose();
  ...
  it("throws on an unsupported metric", () => {
    ...
    const fn = pairwise_distance_matrix as (p: tf.Tensor2D, m: string) => tf.Tensor2D;
