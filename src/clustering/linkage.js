...
exports.update_distance_matrix = update_distance_matrix;
/**
 ...
 */
function update_distance_matrix(D, clusterSizes, i, j, linkage) {
    if (i === j) {
    ...
    const newDistances = new Array(nClusters).fill(0);
    for (let k = 0; k < nClusters; k++) {
        ...
        switch (linkage) {
            ...
            case "ward": {
                ...
                const dij = D[i][j];
                const numerator = (sizeI + sizeK) * dik * dik +
                    (sizeJ + sizeK) * djk * djk -
                    sizeK * dij * dij;
            ...
            default:
                ...
                throw new Error(`Unsupported linkage '${linkage ?? ""}'.`);
        }
        newDistances[k] = dtk;
    }
    /* ------------------------------------------------------------------ */
