...
exports.SpectralClustering = void 0;
...
class SpectralClustering {
    ...
    dispose() {
        ...
        if (this.labels_ != null && this.labels_.dispose instanceof Function) {
    ...
    async fit(_X) {
        ...
        this.dispose();
        /* ---------------------------- 0) Input -------------------------------- */
        const Xtensor = _X instanceof tf.Tensor
            ? tf.cast(_X, "float32")
            : tf.tensor2d(_X, undefined, "float32");
        /* ---------------------------- 1) Affinity ----------------------------- */
        this.affinityMatrix_ = SpectralClustering.computeAffinityMatrix(Xtensor, this.params);
    ...
    static validateParams(params) {
        ...
        if (!isCallable && !SpectralClustering.VALID_AFFINITIES.includes(affinity)) {
            throw new Error(`Invalid affinity '${affinity}'. Must be one of ${SpectralClustering.VALID_AFFINITIES.join(", ")} or a callable.`);
        }
        ...
        if (!isCallable && affinity === "nearest_neighbors") {
            ...
            throw new Error("nNeighbors is only applicable when affinity is 'nearest_neighbors'.");
        }
    ...
    /* ------------------------------------------------------------------- */
    static computeAffinityMatrix(X, params) {
        ...
        const { compute_rbf_affinity, compute_knn_affinity, } = require("../utils/affinity");
        if (affinity === "rbf") {
    ...
    static defaultNeighbors(params, nSamples) {
     ...
     */
    static validateAffinityMatrix(A) {
...
SpectralClustering.VALID_AFFINITIES = [
    "rbf",
    "nearest_neighbors",
    "precomputed",
];
