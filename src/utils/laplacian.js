...
exports.degree_vector = degree_vector;
exports.normalised_laplacian = normalised_laplacian;
exports.jacobi_eigen_decomposition = jacobi_eigen_decomposition;
exports.smallest_eigenvectors = smallest_eigenvectors;
 ...
 * of moderate size.
 */
function jacobi_eigen_decomposition(matrix, { maxIterations = 2000, tolerance = 1e-12, } = {}) {
    ...
    const isApproximatelyDiagonal = () => {
        ...
        for (let i = 0; i < matrix.shape[0]; i++) {
            for (let j = 0; j < matrix.shape[0]; j++) {
                if (i === j)
                    continue;
                if (Math.abs(A[i][j]) > nDiagTolerance)
                    return false;
            }
        }
        return true;
    ...
    if (isApproximatelyDiagonal()) {
        warn("Input matrix is (almost) diagonal – skipped iterative Jacobi rotations for efficiency.");
        const diag = A.map((row, i) => row[i]);
        const V = A.map((_, i) => Array.from({ length: matrix.shape[0] }, (_, j) => (i === j ? 1 : 0)));
    ...
    while (iter < maxIterations && offDiag(D) > tolerance) {
        ...
        if (Math.abs(a_pq) < tolerance) {
            warn(`Jacobi pivot below tolerance (|a_pq|≈${Math.abs(a_pq)}). Skipping rotation.`);
            D[p][q] = D[q][p] = 0;
            iter += 1;
            continue;
        }
    ...
    if (iter === maxIterations) {
        warn(`Jacobi solver did not converge after ${maxIterations} iterations. Final off-diagonal norm: ${offDiag(D)}`);
    }
 ...
 */
function smallest_eigenvectors(matrix, k) {
    ...
    return tf.tidy(() => {
        ...
        const { eigenvectors: vecSorted } = (0, eigen_post_1.deterministic_eigenpair_processing)({
            eigenvalues,
            eigenvectors,
        });
        ...
        const TOL = 1e-2;
        let c = 0;
        for (const v of (0, eigen_post_1.deterministic_eigenpair_processing)({ eigenvalues, eigenvectors }).eigenvalues) {
            if (v <= TOL)
                c += 1;
            else
                break;
        ...
        const selected = Array.from({ length: n }, () => new Array(sliceCols));
